...
이 프로젝트는 실제 운영 환경을 고려한 제한된 Docker 환경에서 진행되었습니다:
...
### API별 성능 개선 결과 (1차 vs 8차 테스트)

| API | 1차 테스트 |  | 8차 테스트 |  | 개선율 |
| --- | --- | --- | --- | --- | --- |
...
이번 프로젝트를 통해 다음과 같은 중요한 교훈을 얻을 수 있었습니다:

1. **성능 최적화는 전체 시스템 관점의 접근이 필요합니다.**
    - 단순히 코드 수정만으로는 성능을 최적화하기 어렵습니다.
    - 데이터베이스, 인프라, 애플리케이션 각 레이어별 최적화가 함께 이루어져야 합니다.
    - 특히 DB 쿼리 최적화와 인덱스 설계가 성능에 큰 영향을 미친다는 것을 배웠습니다.
2. **리소스 관리는 성능 최적화의 핵심입니다.**
    - 커넥션 풀과 쓰레드 풀의 적절한 설정이 매우 중요합니다.
    - "더 많은 리소스 = 더 좋은 성능"이라는 추측은 잘못될 수 있습니다.
    - 특히 커넥션 풀 확장 시도에서 오히려 성능이 저하되는 경험을 통해, 각 리소스의 적정 수준을 찾는 것이 중요하다는 것을 배웠습니다.
    - 로그인과 같은 무거운 작업은 별도의 리소스 관리가 필요하다는 것을 알게 되었습니다.
3. **점진적인 개선과 측정이 중요합니다.**
    - 매 단계별 성능 측정과 개선 효과 분석이 필요합니다.
    - 특정 개선이 다른 부분에 영향을 미칠 수 있다는 것을 배웠습니다.
    - 데이터 기반의 의사결정이 중요합니다.

이러한 경험을 통해 실제 서비스 환경에서의 성능 최적화에 대한 실질적인 이해를 얻을 수 있었습니다.
